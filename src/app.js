...
import { GRAPHCOOL_PROJECT_ID } from './constants/graphcoolConfig'
...
config.addReducer('notification', notificationReducer, { notification: {} });
...
config.setGraphQLEndpoint(`https://api.graph.cool/simple/v1/${GRAPHCOOL_PROJECT_ID}`)
...
if (SERVER) {
  ...
  */
  ...
  config.addMiddleware(async (ctx, next) => {
    ...
    const authHeader = ctx.get('Authorization') || ctx.cookies.get('ReactQLAdmin')
  ...
  config.addApolloMiddleware((req, next) => {
    const jwt = window.localStorage.getItem('ReactQLAdmin')
    req.options.headers = {
      ...req.options.headers,
